...
async def test_is_human_api_error_with_orcid(gateway, mock_session):
    ...
    mock_session.get = MagicMock(side_effect=[mock_resp, orcid_resp])
    gateway._session = mock_session
...
def test_fair_score_empty():
    fair = FAIRMetadata(dpid="test", creation_date="2026-05-30T17:05:36+00:00")
    # Setting explicitly to empty values to reduce the score
    fair.access_protocol = ""
    fair.license = ""
    fair.data_format = ""
    fair.ontology = ""
    fair.version = ""
    score = fair.compute_fair_score()
    assert score < 0.5  # Poucos campos preenchidos
