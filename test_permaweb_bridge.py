...
class TestArweaveDataLayer:
    ...
    @patch('requests.get')
    def test_fetch_data_invalid_tx(self, mock_get, bridge):
...
class TestSecurity:
    ...
    @patch('requests.post')
    def test_process_isolation(self, mock_post, bridge):
        """Test that processes are isolated."""
        mock_post.side_effect = [
            Mock(json=Mock(return_value={"id": "ao-mock-1"})),
            Mock(json=Mock(return_value={"id": "ao-mock-2"}))
        ]
