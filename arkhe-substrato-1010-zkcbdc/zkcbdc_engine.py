...
class AccountState:
    ...
    kyc_level: int = 0               # 0 = unverified, 1 = basic, 2 = complete
...
class ConfidentialTransaction:
    ...
    # Nullifier (prevents double spend without linking transactions)
    nullifier: str
...
class ZKCBCC:
    ...
    def add_to_sanctions_list(self, account_id: str):
        """Adds account to the sanctions list."""
    ...
    def create_transaction(self, sender_priv: str, receiver_pub: str, amount: int) -> ConfidentialTransaction:
        ...
        # Note: in this simplified version, to detect double spending from tests
        # without real ZK proofs, we simply hash the sender_priv.
        # This is a mock behaviour for testing. In reality, the nullifier is derived from the UTXO.
        nullifier = SHA3(f"{sender_priv}:nullifier".encode()).hexdigest()
        ...
        # Simulate anchoring in TemporalChain (923)
...
╠══════════════════════════════════════════════════════════════════╣
  ...
  P1 - Diagnosis: Verifiable money supply without exposure
