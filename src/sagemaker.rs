...
pub async fn create_training_job(
    ...
    let hyper_parameters: HashMap<String, String> = req.hyperparameters.as_object().unwrap().iter().map(|(k, v)| (k.clone(), v.as_str().unwrap_or("").to_string())).collect();
    ...
    sm.create_training_job()
        ...
        .set_hyper_parameters(Some(hyper_parameters))
        .send()
        .await?;
...
pub async fn get_model_artifact(sm: &SmClient, job_name: &str) -> Result<String, Box<dyn std::error::Error>> {
    let resp = sm.describe_training_job()
        ...
        .send()
        .await?;
    Ok(resp.model_artifacts.unwrap().s3_model_artifacts.unwrap_or_default())
}
